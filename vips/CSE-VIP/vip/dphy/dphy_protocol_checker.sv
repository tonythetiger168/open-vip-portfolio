...
//
//============================================================================
//  DPHY Protocol Checker (stub)
//  Minimal activity/validity checker for the DPHY lane of mipi_vip_if.
...
class dphy_protocol_checker extends uvm_component;
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual mipi_vip_if)::get(this, "", "vif", vif))
      `uvm_fatal("CONFIG", "DPHY chk: no vif")
  endfunction
  ...
  task run_phase(uvm_phase phase);
    forever begin
      ...
      if (vif.dphy_hs_rqst) active_cycles++;
      if (vif.dphy_lp_dir && vif.dphy_hs_rqst) err_cnt++;
    end
  endtask

  virtual function void report_phase(uvm_phase phase);
    super.report_phase(phase);
    `uvm_info("DPHY_RPT", $sformatf("DPHY chk | active:%0d err:%0d",
              active_cycles, err_cnt), UVM_LOW)
  endfunction
endclass

`endif // DPHY_PROTOCOL_CHECKER_SV
