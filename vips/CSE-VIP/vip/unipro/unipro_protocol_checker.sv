...
//
//============================================================================
//  UNIPRO Protocol Checker (stub)
//  Minimal activity/validity checker for the UNIPRO lane of mipi_vip_if.
...
class unipro_protocol_checker extends uvm_component;
  ...
  task run_phase(uvm_phase phase);
    forever begin
      ...
      if (vif.unipro_valid) active_cycles++;
      if (vif.unipro_valid && vif.unipro_tc > 3'd7) err_cnt++;
    end
  endtask

  virtual function void report_phase(uvm_phase phase);
    super.report_phase(phase);
    `uvm_info("UNIPRO_RPT", $sformatf("UNIPRO chk | active:%0d err:%0d",
              active_cycles, err_cnt), UVM_LOW)
  endfunction
endclass

`endif // UNIPRO_PROTOCOL_CHECKER_SV
