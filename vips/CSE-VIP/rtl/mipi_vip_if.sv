...
//  The CSE layer (cse_*) is the deepened command/response channel:
//  cse_data is bidirectional (host drives command operand, CSE target
//  drives the response while cse_irq is asserted).
//============================================================================
...
interface mipi_vip_if #(parameter int DATA_WIDTH=32, parameter int LANE_COUNT=4)
  ...
  // packet-type localparams (mirror mipi_vip_pkg::mipi_pkt_type_e encoding)
  localparam logic [5:0] PKT_FS        = 6'h00;
  localparam logic [5:0] PKT_FE        = 6'h01;
  localparam logic [5:0] PKT_RGB565    = 6'h0E;
  localparam logic [5:0] PKT_RAW14     = 6'h2D;
  localparam logic [5:0] PKT_NULL      = 6'h3A;
  localparam logic [5:0] PKT_EMBEDDED  = 6'h3C;
  ...
  // CSE Layer (command/response; cse_data is bidirectional)
  tri  [7:0] cse_data;
  logic      cse_valid;
  logic      cse_ready;
  logic [3:0] cse_cmd;
  ...
  endproperty
  assert_dphy_hs: assert property(p_dphy_hs_rqst) else $error("MIPI_IF: D-PHY HS request timeout");

  property p_cphy_symbol;
    @(posedge clk) disable iff(!rst_n) cphy_symbol_valid |-> cphy_symbol != 0;
  endproperty
  assert_cphy_sym: assert property(p_cphy_symbol) else $error("MIPI_IF: C-PHY invalid symbol");

  property p_mphy_gear;
    @(posedge clk) disable iff(!rst_n) mphy_hs_mode |-> mphy_gear inside {[4'd1:4'd5]};
  endproperty
  assert_mphy_gear: assert property(p_mphy_gear) else $error("MIPI_IF: M-PHY invalid gear");

  property p_unipro_tc;
    @(posedge clk) disable iff(!rst_n) unipro_valid |-> unipro_tc inside {[3'd0:3'd7]};
  endproperty
  assert_unipro_tc: assert property(p_unipro_tc) else $error("MIPI_IF: UniPro TC invalid");

  property p_digrf_frame;
    @(posedge clk) disable iff(!rst_n) digrf_frame_sync |-> digrf_valid;
  endproperty
  assert_digrf: assert property(p_digrf_frame) else $error("MIPI_IF: DigRF frame sync without valid");

  property p_hsi_wake;
    @(posedge clk) disable iff(!rst_n) hsi_wake |-> ##[1:10] hsi_valid;
  endproperty
  assert_hsi_wake: assert property(p_hsi_wake) else $error("MIPI_IF: HSI wake timeout");

  property p_cse_cmd;
    @(posedge clk) disable iff(!rst_n) cse_valid |-> cse_cmd inside {[4'd0:4'd15]};
  endproperty
  assert_cse_cmd: assert property(p_cse_cmd) else $error("MIPI_IF: CSE invalid command");

  // Coverage
  covergroup cg_mipi_if @(posedge clk);
    option.per_instance = 1;
    cp_pkt: coverpoint pkt_type {
      bins fs   = {PKT_FS};
      bins fe   = {PKT_FE};
      bins data = {[PKT_RGB565:PKT_RAW14]};
      bins ctrl = {[PKT_NULL:PKT_EMBEDDED]};
    }
    cp_vc: coverpoint vc {
    ...
    cp_mphy_mode: coverpoint {mphy_ls_mode, mphy_hs_mode} {
      bins ls  = {2'b10};
      bins hs  = {2'b01};
      bins pwm = {2'b00};
    }
