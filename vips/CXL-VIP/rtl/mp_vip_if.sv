...
interface mp_vip_if #(parameter int DATA_WIDTH = 512,
  ...
  //========================================================================
  logic [2:0]             cache_state;      // CXL.cache state (from pm_state)
  assign cache_state = pm_state;
  logic                   mem_req;          // CXL.mem request on the wire
  assign mem_req = valid & sop;
  logic [63:0]            addr;             // request address (from requester)
  assign addr = {48'h0, requester_id};
  logic                   ras_err;          // RAS error rollup
  assign ras_err = ecrc_error | lcrc_error | framing_error | protocol_error;
  logic [31:0]            error_log;        // RAS error log bitmap
  assign error_log = {28'h0, framing_error, protocol_error, lcrc_error, ecrc_error};
  logic [1:0]             security_level;   // IDE security level (not modeled)
  assign security_level = 2'b00;
  logic [1:0]             bifurcation;      // port bifurcation (single x16 model)
  assign bifurcation = 2'b00;
  logic                   ld_on;            // logical device on (link up)
  assign ld_on = link_up;
