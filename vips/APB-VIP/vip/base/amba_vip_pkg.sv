...
// Tiered SVA compliance checker (L1/L2/L3/L4). The SVA content lives in a
// module (class-scope concurrent assertions are illegal per IEEE 1800), so
// this file must be included at compilation-unit scope, outside the package.
`include "sva/apb_compliance_checker.sv"
// Protocol checkers: SVA content lives in modules (class-scope concurrent
// assertions are illegal per IEEE 1800); thin class shells remain. Included
// at compilation-unit scope, before the package, so amba_env can see them.
