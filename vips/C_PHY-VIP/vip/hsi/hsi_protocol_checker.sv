...
//
// hsi_protocol_checker.sv -- HSI protocol checker (generic MAC-level)
`ifndef HSI_PROTOCOL_CHECKER_SV
`define HSI_PROTOCOL_CHECKER_SV

import uvm_pkg::*;
`include "uvm_macros.svh"

class hsi_protocol_checker extends uvm_component;
  `uvm_component_utils(hsi_protocol_checker)
...
covergroup cg_hsi;
  ...
  function new(string name="hsi_chk", uvm_component parent=null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(virtual mipi_vip_if)::get(this, "", "vif", vif))
      `uvm_fatal("CONFIG", "HSI chk: no vif");
  endfunction
  ...
  task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      if (vif.mon_cb.valid && $isunknown(vif.mon_cb.data)) begin
        `uvm_error("HSI_PROTOCOL_CHECKER", "HSI X on data when valid"); proto_err++;
      end
      ...
      if (in_pkt && (cyc - sop_cycle) > 100000) begin
        `uvm_error("HSI_PROTOCOL_CHECKER", "HSI SOP without EOP (timeout)"); proto_err++;
        in_pkt = 0;
      end
  ...
  virtual function void report_phase(uvm_phase phase);
    `uvm_info("HSI_PROTOCOL_CHECKER_RPT", $sformatf("HSI Chk | ERR:%0d PKT:%0d", proto_err, pkt_cnt), UVM_LOW);
  endfunction
endclass
`endif
