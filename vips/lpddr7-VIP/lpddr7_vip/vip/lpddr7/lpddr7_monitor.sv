...
// LPDDR7 Monitor -- decodes DRAM commands from the command/address bus,
// reconstructs transactions (bank/row/col + burst data) and publishes them
// on an analysis port.
`ifndef LPDDR7_MONITOR_SV
`define LPDDR7_MONITOR_SV

class lpddr7_monitor extends uvm_monitor;
  `uvm_component_utils(lpddr7_monitor)
  virtual lpddr7_if vif;
  uvm_analysis_port #(lpddr7_transaction) ap;
  int unsigned txn_count = 0;

  protected lpddr7_transaction open_tr;   // RD/WR waiting for burst data
  protected int              burst_phase = 0;
  protected int              rd_wait = 0;

  function new(string name="lpddr7_monitor", uvm_component parent=null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(virtual lpddr7_if)::get(this, "", "vif", vif))
      `uvm_fatal("CONFIG", "LPDDR7 monitor: vif not found")
  endfunction
    ...
    forever begin
      ...
      if (vif.cs_n === 1'b0) begin
        if (vif.act_n === 1'b0) begin                 // ACT
          tr = lpddr7_transaction::type_id::create("tr");
          tr.cmd = LPDDR7_CMD_ACT;
          ...
          open_tr = lpddr7_transaction::type_id::create("tr");  // WR
          open_tr.cmd = LPDDR7_CMD_WR;
          open_tr.bg = vif.bg; open_tr.ba = vif.ba; open_tr.col = vif.addr[9:0];
          // REACHABILITY: BL is MR-programmed in real LPDDR (not a bus
          // signal); here the driver publishes it on the vif.bl config
          // sideband (same pattern as vif.tCL), so decode it from there.
          open_tr.bl = vif.bl;
          burst_phase = 1;
        end else if (vif.ras_n === 1'b1 && vif.cas_n === 1'b0 && vif.we_n === 1'b1) begin
          open_tr = lpddr7_transaction::type_id::create("tr");  // RD
          open_tr.cmd = LPDDR7_CMD_RD;
          open_tr.bg = vif.bg; open_tr.ba = vif.ba; open_tr.col = vif.addr[9:0];
          rd_wait = (vif.tCL > 0 ? vif.tCL : 6) + 1;
        end else if (vif.ras_n === 1'b0 && vif.cas_n === 1'b1 && vif.we_n === 1'b0) begin
          tr = lpddr7_transaction::type_id::create("tr");       // PRE
          tr.cmd = LPDDR7_CMD_PRE;
          tr.bg = vif.bg; tr.ba = vif.ba;
          txn_count++;
          `uvm_info("MON", {"mon: ", tr.convert2string()}, UVM_HIGH)
          ap.write(tr);
        end else if (vif.ras_n === 1'b0 && vif.cas_n === 1'b0 && vif.we_n === 1'b1) begin
          tr = lpddr7_transaction::type_id::create("tr");       // REF
          tr.cmd = LPDDR7_CMD_REF;
          txn_count++;
          `uvm_info("MON", {"mon: ", tr.convert2string()}, UVM_HIGH)
          ap.write(tr);
        end
  ...
  function void report_phase(uvm_phase phase);
    `uvm_info("MON", $sformatf("lpddr7 monitor observed %0d transactions", txn_count), UVM_LOW)
  endfunction
endclass
`endif
