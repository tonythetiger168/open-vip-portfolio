...
// LPDDR7 Driver -- issues DRAM command encodings (ACT/RD/WR/PRE/REF/NOP)
// with bank/row/column addressing and tRCD/tRP/tRAS/tCL timing concepts.
`ifndef LPDDR7_DRIVER_SV
`define LPDDR7_DRIVER_SV

class lpddr7_driver extends uvm_driver #(lpddr7_transaction);
  `uvm_component_utils(lpddr7_driver)
  virtual lpddr7_if vif;
  uvm_analysis_port #(lpddr7_transaction) drv_ap;  // expected items for scoreboard
  ...
  function new(string name="lpddr7_driver", uvm_component parent=null);
  ...
  function void build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(virtual lpddr7_if)::get(this, "", "vif", vif))
      `uvm_fatal("CONFIG", "LPDDR7 driver: vif not found")
  endfunction
  ...
  task drive_write(lpddr7_transaction tr);
  ...
  task drive_read(lpddr7_transaction tr);
    vif.bl = tr.bl;                                    // publish BL sideband for monitor
  ...
  task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      case (tr.cmd)
        ...
        LPDDR7_CMD_PRE: drive_pre(tr.bg, tr.ba);
        LPDDR7_CMD_REF: drive_ref();
        default:         drive_nop();
      endcase
